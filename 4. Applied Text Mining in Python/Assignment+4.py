...
def similarity_score(s1, s2):
    ...
    s = []
    for synset1 in s1:
        similarity_scores = [score for score in [synset1.path_similarity(synset2) for synset2 in s2] if score is not None]
        if similarity_scores:
            s.append(max(similarity_scores))
...
a = doc_to_synsets('Fish are nvqjp friends.')
print(a)
...
size = [len(doc) for doc in paraphrases["D1"]]
size2 = [len(doc) for doc in paraphrases["D2"]]
print(size)
print(size2)
paraphrases.head()
...
def most_similar_docs():
    ...
    similar_docs = [(paraphrases["D1"], paraphrases["D2"], document_path_similarity(paraphrases["D1"], paraphrases["D2"]))
                        for index, paraphrases in paraphrases.iterrows()]
    return max(similar_docs, key = lambda item:item[2])
